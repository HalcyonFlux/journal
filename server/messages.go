...
func NewSender(conn net.Conn, cmd string, args Args, respond, close bool) SocketMessage {
	return newMessage(conn, cmd, args, &Response{}, respond, close)
}

// NewReceiver creates a blank message for the receiver
func NewReceiver(conn net.Conn) SocketMessage {
	return newMessage(conn, "", Args{}, &Response{}, true, true)
}

// newMessage creates a new socket message with default options
func newMessage(conn net.Conn, cmd string, args Args, resp *Response, respond, close bool) *socketMessage {
...
// Options set some options on the sending/receiving
func (s *socketMessage) Options(maxLength int, timeout time.Duration, respond, close bool) {
	s.Respond = respond
	s.Close = close
...
func (s *socketMessage) Send() error {
...
func (s *socketMessage) Receive() error {
...
// GetResponse returns message's response
func (s *socketMessage) GetResponse() *Response {
	return s.Response
}

// SetResponse sets message's response
func (s *socketMessage) SetResponse(resp *Response) {
	s.Response = resp
}

// GetCmd returns message's command
func (s *socketMessage) GetCmd() string {
	return s.Cmd
}

// GetArgs returns message's command arguments
func (s *socketMessage) GetArgs() Args {
	return s.Args
}

// ShouldRespond informs the message handler that a response is expected
func (s *socketMessage) ShouldRespond() bool {
	return s.Respond
}

// ShouldClose informs the message handler that there's not going to be any
// further communication and the connection can be closed
func (s *socketMessage) ShouldClose() bool {
	return s.Close
}
